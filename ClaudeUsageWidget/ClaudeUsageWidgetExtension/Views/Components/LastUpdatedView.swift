...
#Preview {
    VStack(spacing: 16) {
        // Recent update
        LastUpdatedView(lastUpdated: Date())

        // Few minutes ago
        LastUpdatedView(lastUpdated: Date().addingTimeInterval(-300))

        // An hour ago
        LastUpdatedView(lastUpdated: Date().addingTimeInterval(-3600))

        // Multiple hours ago
        LastUpdatedView(lastUpdated: Date().addingTimeInterval(-7200))

        // Yesterday
        LastUpdatedView(lastUpdated: Date().addingTimeInterval(-86400))

        // Static string version
        StaticLastUpdatedView(timeString: "just now")
    }
    .padding()
